...
def jitter_gt_centroids(gt_centroids):
    """jitter the gt_centroids, before adding thesm into rois, to be more robust for seg
    Args:
...
def generate_training_centroids(gt_centroids, bn_seg, off, training_flag):
    ...
    gt_centroids = torch.from_numpy(gt_centroids).unsqueeze(0)
    gt_centroids = gt_centroids[:, :, :].cpu().data.numpy()
